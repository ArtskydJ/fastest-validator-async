 ...
 * @param {Object} schema
 */
Validator.prototype.validate = async function(obj, schema) {
	const check = this.compile(schema);
	return await check(obj);
};
...
Validator.prototype.compileSchemaObject = function(schemaObject) {
	...
	for (let i = 0; i < compiledObject.properties.length; i++) {
		...
		if (Array.isArray(property.compiledType)) {
			sourceCode.push(`res = await this.checkSchemaType(${propertyValueExpr}, properties[${i}].compiledType, propertyPath, value);`);
		} else {
			sourceCode.push(`res = await this.checkSchemaRule(${propertyValueExpr}, properties[${i}].compiledType, propertyPath, value);`);
		}
	...
	compiledObject.compiledObjectFunction = new AsyncFunction("value", "properties", "path", "parent", sourceCode.join("\n"));
...
Validator.prototype.checkSchemaObject = async function(value, compiledObject, path, parent) {
	if (compiledObject.cycle) {
		...
		const result = await this.checkSchemaObjectInner(value, compiledObject, path, parent);
		compiledObject.objectStack.pop();
		return result;
	} else {
		return await this.checkSchemaObjectInner(value, compiledObject, path, parent);
	}
};

Validator.prototype.checkSchemaObjectInner = async function(value, compiledObject, path, parent) {
	return await compiledObject.compiledObjectFunction.call(this, value, compiledObject.properties, path, parent);
...
Validator.prototype.checkSchemaType = async function(value, compiledType, path, parent) {
	...
	if (Array.isArray(compiledType)) {
		...
		for (let i = 0; i < checksLength; i++) {
			...
			const res = await this.checkSchemaRule(value, compiledType[i], path, parent);
	...
	return await this.checkSchemaRule(value, compiledType, path, parent);
};

Validator.prototype.checkSchemaArray = async function(value, compiledArray, path, parent) {
	const errors = [];
	const valueLength = value.length;

	for (let i = 0; i < valueLength; i++) {
		const itemPath = (path !== undefined ? path : "") + "[" + i + "]";
		const res = await this.checkSchemaType(value[i], compiledArray, itemPath, value, parent);
...
Validator.prototype.checkSchemaRule = async function(value, compiledRule, path, parent) {
	...
	const res = await compiledRule.ruleFunction.call(this, value, schemaRule, path, parent);
	...
	if (compiledRule.dataFunction !== null) {
		return await compiledRule.dataFunction.call(this, value, compiledRule.dataParameter, path, parent);
	}
